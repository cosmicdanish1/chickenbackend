...
async function addPurchaseChargesColumns() {
  ...
  try {
    ...
    for (const column of columnsToAdd) {
      if (!existingColumnNames.includes(column.name)) {
        const alterQuery = `ALTER TABLE purchase_orders ADD COLUMN ${column.name} ${column.type};`;
    ...
    // Update existing records to calculate gross_amount and net_amount
    const updateQuery = `
